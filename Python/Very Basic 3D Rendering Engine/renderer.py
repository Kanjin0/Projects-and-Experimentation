...
def rasterize_triangle_tiled_lighting_material(p0, p1, p2, z0, z1, z2, pixels,
                                               n0=None, n1=None, n2=None,
                                               material=None):
    if material is None:
    ...
    # ---- Default values for Phong ----
    use_phong = False
    spec_color = (1.0, 1.0, 1.0)
    shininess = 1.0

    # ---- Lighting constants (always needed) ----
    ambient = AMBIENT_STRENGTH
    ldx, ldy, ldz = LIGHT_DIR
    l_len = sqrt(ldx*ldx + ldy*ldy + ldz*ldz)
    if l_len > 1e-10:
        ldx /= l_len; ldy /= l_len; ldz /= l_len
    vx, vy, vz = 0.0, 0.0, 1.0   # view direction

    # ---- Check if Phong is active ----
    if mode == SHADING_PHONG and n0 is not None and n1 is not None and n2 is not None:
        use_phong = True
    ...
    # ---- Flat colour ----
    r_base = int(material.diffuse[0] * 255)
    g_base = int(material.diffuse[1] * 255)
    b_base = int(material.diffuse[2] * 255)
                    ...
                    if u >= -1e-6 and v >= -1e-6 and w >= -1e-6:
                        ...
                        if depth < zb[idx]:
                            ...
                            if not use_phong:
                                ...
                                # Final color = diffuse * intensity + specular * spec
                                r = int((material.diffuse[0] * intensity + spec_color[0] * spec) * 255)
                                g = int((material.diffuse[1] * intensity + spec_color[1] * spec) * 255)
                                b = int((material.diffuse[2] * intensity + spec_color[2] * spec) * 255)
                                pixels[x, y] = (min(255, r), min(255, g), min(255, b))