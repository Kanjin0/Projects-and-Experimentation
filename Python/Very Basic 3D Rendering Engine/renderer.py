import pygame
from engine_config import VERTEX_COLOR,EDGE_COLOR, FACE_COLOR, DRAW_FACES
...
def point(point:Point2D):
    size = 10
    pygame.draw.rect(window, VERTEX_COLOR,(point.x - size/2, point.y- size/2,size,size)) # type: ignore (it's for the window being undefined)

#Draw a line conecting both points specified
def line(point1:Point2D, point2:Point2D):
    size = 2
    if DRAW_FACES: size = 5
    pygame.draw.line(window,EDGE_COLOR, (point1.x, point1.y), (point2.x, point2.y),size) # type: ignore (it's for the window being undefined)

#Draw a face of the model conecting all points specified
def face(points:list):
    pygame.draw.polygon(window,FACE_COLOR,points) # type: ignore (it's for the window being undefined)
